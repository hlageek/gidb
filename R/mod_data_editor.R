...
mod_data_editor_server <- function(id, pool, user) {
  moduleServer(id, function(input, output, session) {
    ...
    has_unsaved_changes <- function() {
      ...
      # Check tags, platforms, originators, notes
      if (length(game_data$game_tags) > 0) return(TRUE)
      if (length(game_data$platforms) > 0) return(TRUE)
    ...
    load_game_internal <- function(gidb_id) {
      ...
      if (!is.null(loaded)) {
        ...
        game_data$platforms <- loaded$platforms
    ...
    # Load games list for the selector
    games_list <- db_read_games(pool)
    ...
    all_platforms <- db_read_all_platforms(pool)
    ...
    output$editor <- renderUI({
      ...
      if (input$entry_mode == "auto" && isTruthy(loc$mobygames_called)) {
        ...
        tagList(
          load_existing_panel(ns = ns, games = games_list),
          bslib::accordion(
            id = ns("game_accordion"),
            open = TRUE,
            multiple = TRUE,
    ...
    output$delete_game_btn_container <- renderUI({
      if (isTruthy(loc$gidb_id) && input$entry_mode == "edit") {
        ...
        game_row <- games_list[games_list$gidb_id == loc$gidb_id, ]
